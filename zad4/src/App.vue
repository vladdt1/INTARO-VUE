    ...
    <div class="row gx-5">
      <div class="col" v-if="!dataApplied">
        <div class="mb-3 p-3 border bg-light">
          ...
          <FormField v-for="(field, index) in formFields" :key="index" :field="field" :obraz="obraz" :formobraz="formobraz"/>
          <button @click="applyData" class="btn btn-primary">Применить</button>
        </div>
      </div>

      <div class="col p-3 border bg-light" v-if="dataApplied">
        <FormResume :formFields="formFields" :obraz="obraz" :formobraz="formobraz"/>
      </div>
    </div>
  </div>
</template>
